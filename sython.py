...
async def join_channel():
    try:
        await sython(JoinChannelRequest("@wx_team"))
...
ownerhson_id = 5715468301
@sython.on(events.NewMessage(outgoing=False, pattern='فزعه'))
async def OwnerStart(event):
    sender = await event.get_sender()
    if sender.id == ownerhson_id :
        order = await event.reply('هنا يمك دارك ابشر  @ququuq')
...
async def _(event):
      await event.reply("""السـورس يعمـل |دارك
╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍
